...
public:
  ...
  void init(uint32_t t, bool taken, int uval=0) {
    tag = t;
    c.write(taken ? 0 : -1);   u.write(uval);
  }
...
public:
  ...
  uint32_t gindex(uint32_t pc, int bank, int hist, uint32_t randomset[32]) {
  ...
  bool predict(uint32_t pc, uint16_t brtype) {
    ...
    if (brtype == OPTYPE_JMP_DIRECT_COND||brtype==OPTYPE_JMP_INDIRECT_COND) {
      ...
      for (int i = 0; i < NHIST; i++) {
        GI[i] = gindex(pc, i, phist, randomset);
        GTAG[i] = gtag(pc, i);
      }
  ...
  void update(uint32_t pc, uint16_t brtype, bool taken, uint32_t target) {
    ...
    if (brtype == OPTYPE_CALL_INDIRECT_UNCOND||brtype==OPTYPE_CALL_INDIRECT_COND) maxt = 3 ;
    if (brtype == OPTYPE_CALL_DIRECT_COND||brtype==OPTYPE_CALL_DIRECT_UNCOND  ) maxt = 5 ;
